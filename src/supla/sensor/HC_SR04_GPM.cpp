...
//  interrupts();
  ...
  return failCount <= 3 ? static_cast<double>(value) / 100.0
                        : -1; //DISTANCE_NOT_AVAILABLE;
}

void HC_SR04_GPM::setMinMaxIn(int16_t minIn, int16_t maxIn) {
  _minIn = minIn;
  _maxIn = maxIn;
}

void HC_SR04_GPM::setMinMaxOut(int16_t minOut, int16_t maxOut) {
  _minOut = minOut;
  _maxOut = maxOut;
}
