...
std::unique_ptr<SupletFifoInput> supletFifo;

}  // namespace

bool setupLinuxSupletRuntime(Supla::Config *config) {
  return setupSupletRuntime(config);
}

bool initLinuxSupletFifo(const std::string &path) {
  supletFifo = std::make_unique<SupletFifoInput>(path);
  return supletFifo->init();
}

void iterateLinuxSupletFifo() {
  if (supletFifo != nullptr) {
    supletFifo->iterate();
  }
}
