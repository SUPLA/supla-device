...
bool VolumeParameters::handleResponse(const char* key,
                                                const char* value) {
  auto cfg = Supla::Storage::ConfigInstance();
  if (strcmp(key, Supla::Html::VolumeCfgTag) == 0) {
    ...
    cfg->getUInt8(Supla::Html::VolumeCfgTag, &currentValue);
    if (currentValue > 100) {
      currentValue = 100;
    }

    if (param != currentValue) {
      cfg->setUInt8(Supla::Html::VolumeCfgTag, static_cast<uint8_t>(param));
