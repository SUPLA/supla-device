...
TEST_F(SwUpdateTests, FirmwareCheckAndNormalUpdate) {
  ...
  EXPECT_CALL(srpc, srpc_getdata(_, _, _))
      .WillOnce([&calcfgRequest](void *, TsrpcReceivedData *rd, unsigned _supla_int_t) {
  ...
  EXPECT_CALL(srpc, srpc_ds_async_device_calcfg_result(_, _)).WillOnce(
      [](void *, TDS_DeviceCalCfgResult *result) {
